...
export const Numberer = ({ dictionary }: { dictionary: NumbererToolDict }) => {
      ...
      {!file ? (
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/3 flex flex-col justify-center space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="startNumber">{dictionary.start_at_label}</Label>
            ...
            <Button onClick={handleAddNumbers} disabled={isProcessing} className="w-full text-lg py-6">
              ...
              {isProcessing ? dictionary.processing : dictionary.button_add}
            </Button>
          </div>
        </div>
      )}
      {outputPdfUrl && (
        <div className="mt-6 text-center p-6 bg-green-50 dark:bg-green-900/20 border rounded-lg">
          ...
          <Button asChild size="lg">
            <a href={outputPdfUrl} download={`numbered-${file?.name}`}>{dictionary.download_button}</a>
          </Button>
        </div>
      )}
      </CardContent>
