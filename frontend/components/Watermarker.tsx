...
export const Watermarker = ({ file, onReset }: WatermarkerProps) => {
  ...
  const handleAddWatermark = async () => {
    ...
    try {
      ...
      for (const page of pages) {
        ...
        // Using a simpler, direct method for drawing that is more reliable.
        if (style === 'tiled') {
            const gap = 150;
        ...
        <div className="mt-6 text-center p-6 bg-green-50 dark:bg-green-900/20 border rounded-lg">
          <h3 className="text-2xl font-semibold text-green-800 dark:text-green-300 mb-4">Watermark Added!</h3>
           <div className="flex justify-center items-center gap-4">
            ...
            <Button onClick={onReset} variant="outline" size="lg">Watermark Another PDF</Button>
           </div>
        </div>
      )}
    </>
